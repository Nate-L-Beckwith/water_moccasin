...
def create_folder_for_iso(download_dir, iso_name):
    ...
    The directory name is sanitized to avoid issues with path separators.
    """
...
def write_info_file(folder_path, info_content):
    """
    Write or update an information file in the specified directory with details about the download.
    """
    info_file_path = os.path.join(folder_path, 'info.txt')
    with open(info_file_path, 'w') as file:
        file.write(info_content)

def download_file(name, size, download_dir, progress_dict, standard_size=1459978240):
    """
    Download a single file using streaming to handle large files. Progress is tracked with a tqdm progress bar.
    """
...
def download_files_from_xml(xml_url, download_dir):
    """
    Download all files listed in the specified XML. Uses concurrent downloads to speed up the process.
    """
    response = requests.get(xml_url)
    root = ET.fromstring(response.content)
    files = [(file.get('name'), int(file.get('size', -1)), download_dir) for file in root.findall('.//file') if file.get('name')]

    # Dictionary to store the progress bar positions for each download
    ...
    with ThreadPoolExecutor(max_workers=5) as executor:
        ...
        overall_progress = tqdm(total=len(files), desc="Overall Progress", position=len(files) + 1)
        for future in futures:
            result = future.result()
            overall_progress.update(1)  # Update the overall progress after each file is processed
